...
if (isset($_POST['submit'])) {
    ...
    if (empty($location) || empty($date) || empty($time)) {
        header("Location: ccua_create.php?error=Please fill it all out");
        exit();
    } else {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $insertQuery = "insert into `coastal_cu_admin` (user_id, Location, Date, Time) values ('$user_id', '$location', '$date', '$time')";
...
<head>
    ...
    <link rel="stylesheet" href="css/ccua.css">
...
<body>
    <header class="header">
        <a href="coastalcuadmin.php" class="logo"><i class="fas fa-globe-asia"></i> humanCares</a>
    </header>

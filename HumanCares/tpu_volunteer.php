...
if (isset($_POST["submit"])) {
    ...
    $sql = "SELECT location, date, time FROM tree_planting_admin WHERE treepid = ?";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("i", $schedule_id);
    $stmt->execute();
    $result = $stmt->get_result();
    if (empty($activity) || empty($firstname) || empty($lastname) || empty($address) || empty($mobileno)) {
        header("Location: tpu_volunteer.php?error=Please fill it all out");
        exit();
    } else if (strlen($mobileno) < 11) {
        header("Location: tpu_volunteer.php?error=Phone number must be 11 digit");
        exit();
    } else if (strlen($mobileno) > 11) {
        header("Location: tpu_volunteer.php?error=Phone number must be 11 digit");
        exit();
    ...
    if ($result->num_rows == 1) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $insertQuery = "INSERT INTO tree_planting_user (user_id, treepid, activity, firstname, middlename, lastname, address, mobileno) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
<head>
    ...
    <title>Tree Planting Volunteer</title>
...
<body>
    ...
    <header class="header">
        <a href="treeplantinguser.php" class="logo"><i class="fas fa-globe-asia"></i> humanCares</a>
    </header>
    <br><br><br>
    <!-- header section ends here -->

    <h1 class="heading"> Tree Planting <span style="color:#16a085">Volunteer</span></h1>
    ...
    <div class="container">
        <form method="post">
            ...
            <div class="form-group my-3">
                ...
                <?php
                include 'connection.php';
                $sql = "SELECT treepid, location, date, time FROM tree_planting_admin";
